...
LocalFile = function(ui, data, title, temp)
{
	...
	console.log('LocalFile data: ' + data);
	this.title = title;
...
{
	console.log('execute LocalFile.prototype.saveFile');
	this.title = title;
	...
	{
		// 解密解压，使保存的文件可阅读
		var filedoc = mxUtils.parseXml(data);
		var  mxfile = filedoc.firstChild;
		var diagram = mxfile.firstChild;
		var realdata = mxUtils.getTextContent(diagram);
		var graph = this.ui.createTemporaryGraph();
		var deRealdata = graph.decompress(realdata);
		diagram.innerHTML = deRealdata;
		data = mxUtils.getPrettyXml(mxfile);
